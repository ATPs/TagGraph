...
#slin 20180720  rename to VerifyVM.py
...
TABLEFIELDSmetaProteome="(ScanF varchat(50),Charge int,MatchingTagLength int,SpectrumProbabilityScore decimal(23,20),Specificity varchar(40),Modified int,PPMError decimal(14,10),ModSize varchar(50),NumMods int,UniqueSiblings int,ContextModVariants int,NumModOccurrences int,ModClass int,NumSingleModsFound int,Taxon varchar(100),TaxonProteinCount int,Context varchar(100),ModContext varchar(100),ModTuple varchar(300),EMProbability decimal(23,20),OneSubtractlg10EM decimal(23,20),PosSpectrumProb decimal(23,20),NegSpectrumProb decimal(23,20),PosModProb decimal(23,20),NegModProb decimal(23,20),PosContextProb decimal(23,20),NegContextProb decimal(23,20),PosDBMatchProb decimal(23,20),NegDBMatchProb decimal(23,20),PosProteinCountProb decimal(23,20),NegProteinCountProb decimal(23,20),PosMissedCleavageProb decimal(23,20),NegMissedCleavageProb decimal(23,20),PosPPMErrorProb decimal(23,20),NegPPMErrorProb decimal(23,20),PosTaxonProb decimal(23,20),NegTaxonProb decimal(23,20),PosProb decimal(23,20),NegProb decimal(23,20),Log10EM decimal(23,20),EM decimal(23,20))"

def createEMFullTable(conn,metaProteome):
    if metaProteome:
        sqlTblFields=TABLEFIELDSmetaProteome
    else:
        sqlTblFields=TABLEFIELDS
    conn.execute("create table if not exists EMFull"+sqlTblFields);
    conn.execute("delete from EMFull");

def readFile(infile,startLine,endLine,metaProteome,delimiter=' '):
    dataList=[]
    if metaProteome:
        Log10EMPos=19
    else:
    ...
    with open(infile,"r") as f:
        for line in itertools.islice(f,startLine,endLine):
            ...
            Log10EM=math.log(float(splitLine[Log10EMPos]))
            if (splitLine[Log10EMPos+1]>-1*Log10EM):
                EM=splitLine[Log10EMPos+1]
            else:
                EM=Log10EM
...
def populateEMTable(outputFolder,conn,metaProteome):
    ...
    for i in range(0,iteration):
        ...
        if endVal>lineCount:
            endVal=lineCount
        dataList=readFile(infile,startVal,endVal,metaProteome,'	')
        loadEMFullTable(conn,dataList,metaProteome)

def loadEMFullTable(conn,dataList,metaProteome):
    if metaProteome:
        conn.executemany("insert into EMFull(ScanF,Charge,MatchingTagLength,SpectrumProbabilityScore,Specificity,Modified,PPMError,ModSize,NumMods,UniqueSiblings,ContextModVariants,NumModOccurrences,ModClass,NumSingleModsFound,Taxon,TaxonProteinCount,Context,ModContext,ModTuple,EMProbability,OneSubtractlg10EM,PosSpectrumProb,NegSpectrumProb,PosModProb,NegModProb,PosContextProb,NegContextProb,PosDBMatchProb,NegDBMatchProb,PosProteinCountProb,NegProteinCountProb,PosMissedCleavageProb,NegMissedCleavageProb,PosPPMErrorProb,NegPPMErrorProb,PosTaxonProb,NegTaxonProb,PosProb,NegProb,Log10EM,EM) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",dataList)
    else:
...
def verifyEM(outputFolder,metaProteome=False):
    dbFile=outputFolder+"/results.db"
    conn=lite.connect(dbFile)
    cursor=conn.cursor()
    createEMFullTable(conn,metaProteome)
    populateEMTable(outputFolder,conn,metaProteome)
    ...
    writeEMResult(outputFolder,result)
    return result

def writeEMResult(outputFolder,result):    
    emTestFile=outputFolder+"/verifyEM.txt"
    fout=open(emTestFile,'w')
    fout.write(result)
    fout.close()
...
if __name__ == '__main__':
    if (len(sys.argv)<2):
        ...
        #python VerifyEM.py /lab/samba/shared/Users/Sarah/A375 metaProteome=[True,False]
        #python VerifyEM.py /home/slin/TagGraph/sampleInputFiles/TG/fusion2Frac True
        #python VerifyEM.py /home/slin/TagGraph/Pluck/Out_f1_0123 False
        sys.exit(2)
    metaProteome=False
    outputFolder=sys.argv[1]
    if len(sys.argv)==3:
        metaProteome=sys.argv[2]
    result=verifyEM(outputFolder,metaProteome)
    writeEMResult(outputFolder,result)
    print(result)