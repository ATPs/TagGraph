#slin 201808    update code for Metaproteome
import sys
import os
        ...
        if len(match_data) > 0:
            ...
            for item_key, max_composite_score in max_composite_scores:
                ...
                for match, spectrum_score, pm, composite_score, proteins, write_data_ind in results:
                    ...
                    scanData['Proteins'] = list(proteins)[:2]  #slin: limit to show first 2 protein 
                    ...
                    if i<=5:
                        if not options.splittaxon:
                            ourstr='\t'.join([str(scanData[col]) for col in cols]) + '\n'
                            #outFile.write('\t'.join([str(scanData[col]) for col in cols]) + '\n') 
                            #print str(scanData['Proteins'])
                            #print str(scanData['Matching Tag Length'])
                            #print str(scanData['De Novo Peptide'])
                            #print('\t'.join([str(scanData[col]) for col in cols]) + '\n')
                            outFile.write(ourstr)
                            #print ourstr
                        else:
