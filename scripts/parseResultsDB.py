...
def splitModsSubtition(newDic,allMods,modContextList,contextList,maxModLength,deNovoPeptide,staticModsList,NTERM,CTERM,dictDiffMods,dictDiffModsMass,dictDiffModsName,dictStaticMods,dictStaticModsName,dictDiffModsWithOriginal,dictStaticCNTermModsName):
    ...
    for i in xrange(len(allMods)):
        massArray=modsArray(allMods[i],massArray)
...
def insertBaseDataForPPMCalculation(conn,idList,scanList,chargeList,PPMList,newPPMList,EMProbabilityList,oneSubtractlg10EMList,finalPeptideList,tenPowerNegOneSubtractlg10EMList):
    dataList=[]
    for i in xrange(len(chargeList)):
        dataList.append((idList[i],scanList[i],chargeList[i],PPMList[i],newPPMList[i],round(newPPMList[i],1),EMProbabilityList[i],oneSubtractlg10EMList[i],tenPowerNegOneSubtractlg10EMList[i],finalPeptideList[i][2:-2]))
    conn.executemany("INSERT INTO baseDataForPPMCalculation(id,scan,charge,PPM,newPPM,roundedPPM,EMProbability,oneSubtractlg10EM,tenPowerNegOneSubtractlg10EM,finalPeptide) VALUES (?,?,?,?,?,?,?,?,?,?)",dataList)
    conn.commit()
...
def createPPMRangesFDRTables(conn):
    ...
    conn.execute("CREATE TABLE cleanDataFDR(id int,scan int,charge int,fractionID int,PPM decimal(14,8),roundedPPM decimal(10,1),EMProbability decimal(14,10),oneSubtractlg10EM decimal(14,10),newEM decimal(8,2),finalPeptide varchar(512),FDR decimal(8,4))")
...
def createPPMTables(conn):
    ...
    conn.commit()
    conn.execute("CREATE TABLE baseDataForPPMCalculation(id int,scan int,charge int,PPM decimal(14,8),newPPM decimal(14,8),roundedPPM decimal(10,1),EMProbability decimal(14,10),OneSubtractlg10EM decimal(14,10),tenPowerNegOneSubtractlg10EM decimal(14,10),finalPeptide varchar(512))")
...
def insertCleanDataFDR(conn,fractionID,ppmRangeUpperBound,ppmRangeLowerBound):  #per fraction now.
    query="insert into cleanDataFDR(id,scan,fractionID,charge,PPM,roundedPPM,EMProbability,oneSubtractlg10EM,finalPeptide) select id,scan,"+str(fractionID)+",charge,ppm,roundedPPM,EMProbability,oneSubtractlg10EM,FinalPeptide from baseDataForPPMCalculation"
    conn.execute(query)
    conn.commit()
...
def getScansPerFraction(conn,fractionID,dictDiffMods,dictStaticMods):
    cursor=conn.cursor()
    query="SELECT r.id,r.scan,c.contextILCorrected,r.mods,r.mod_context,r.de_novo_peptide,r.obs_mh,r.em_probability,r.log_em_probability,r.charge,r.ppm FROM result r, (select * from correctedILPeptide group by scan, fractionID,EMProbability, ContextILCorrected, proteins) c WHERE r.top_result=1 and r.fraction_id="+str(fractionID)+" and r.scan=c.scan and r.fraction_id=c.fractionID ORDER BY r.id"
    cursor.execute(query)
...
def writeHeader(fout,nonMp="1"):
    if nonMp=="1":
        ...
        newHeader="ScanF\tCharge\tRetention Time\tObs M+H\tTheo M+H\tPPM\tPPM Upper Bound\tPPM Lower Bound\tPPM In Range\tFDR\tEM Probability\t1-lg10 EM\tSpectrum Score\tAlignment Score\tComposite Score\tUnique Siblings\tContext Mod Variants\tNum Mod Occurrences\tContext\tMod Context\tMods\tMod Ambig Edges\tMod Ranges\tProteins\tParsimony Protein\tParsimony Taxon\tTaxon Info\tDe Novo Peptide\tDe Novo Score\tMatching Tag Length\tNum Matches\tCovering Set Size\tCombined NSAF\tN-terminus\tFinal Peptide\tC-terminus"
...
def processProteinForPrintout(protein,DisplayProteinNum):
    ...
    if cntProtein>0:
        ...
        for i in range(0,int(proteinCnt)):
            if i==0:
                newProteinsub=prependSquarBracket+splitProtein[i][:MaxProtenLengthEach]
            else:
                newProteinsub=newProteinsub+" ||| "+splitProtein[i][:MaxProtenLengthEach]
        newProtein=newProtein+newProteinsub+postpendSquarBracket
    return newProtein    
    
def processProteinForPrintout_MP(protein,DisplayProteinNum,taxonIDs,taxonNames,taxionIDName):
    newProtein=""
    splitVia=" ||| "
    splitProtein=protein.split(splitVia)
    totalProtein=len(splitProtein)
    cntProtein=totalProtein 
    displayCnt=int(DisplayProteinNum)
    proteinCnt=displayCnt
    prependSquarBracket="["
    postpendSquarBracket="]"
    ...
    if cntProtein>0:
        ...
        for i in range(0,int(proteinCnt)):
            if i==0:
                #newProteinsub=prependSquarBracket+splitProtein[i][:MaxProtenLengthEach]
                newProteinsub=prependSquarBracket+splitProtein[i]
            else:
                #newProteinsub=newProteinsub+" ||| "+splitProtein[i][:MaxProtenLengthEach]
                newProteinsub=newProteinsub+" ||| "+splitProtein[i]
        newProtein=newProtein+newProteinsub+postpendSquarBracket
    foundTaxon=0
    for j in range(0,len(taxonIDs)):
        if foundTaxon==1:
            break
        taxonID=taxonIDs[j]
        taxonName=taxonNames[j]
        for k in range(0,totalProtein):
            curProtein=splitProtein[k]
            if (foundTaxon==0 and(curProtein.find(taxonID)>=0)):
                parsimonyProtein=curProtein
                parsimonytaxon=taxonID
                foundTaxon=1
    if parsimonyProtein=="":
        parsimonyProtein=splitProtein[0]
        parsimonytaxon=curProtein.split(" TAXON=")[1]
    for k in range(0,totalProtein):
        taxon=str(splitProtein[k].split(" TAXON=")[1])
        if taxon in taxonD:
          taxonD[taxon]+=1
        else:
          taxonD[taxon]=1
    for key, value in taxonD.items():
        if taxonInfo=="":
            if key in taxionIDName:
                taxonInfo="["+taxionIDName[str(key)]+"] ID:"+str(key)+": "+str(value)
            else:
                taxonInfo="ID:"+str(key)+": "+str(value)
        else:
            if key in taxionIDName:
                taxonInfo+="; ["+taxionIDName[str(key)]+"] ID:"+str(key)+": "+str(value)
            else:
                taxonInfo+="; ID:"+str(key)+": "+str(value)
    return newProtein,parsimonyProtein,parsimonytaxon,taxonInfo

def createIndexesViews(conn):
    cursor=conn.cursor()
    ...
    cursor.execute("CREATE INDEX IF NOT EXISTS result_3 on result(id,top_result,scan,fraction_id)")
    ...
    cursor.execute("CREATE INDEX IF NOT EXISTS result_1Top_resultFraction_id on result(id,top_result,fraction_id)")
    cursor.execute("CREATE INDEX IF NOT EXISTS finalPeptidePPMInfo_v1 ON finalPeptidePPMInfo(id,fractionID,scan,newTheoMass,newPPM,nTerm,finalPeptide,cTerm)")
    cursor.execute("CREATE INDEX IF NOT EXISTS cleanDataFDR_v1 ON cleanDataFDR(id,fractionID,scan)")
    cursor.execute("CREATE INDEX IF NOT EXISTS modsPerScan_v1 ON modsPerScan(id,fractionID,scan)")
    cursor.execute("CREATE INDEX IF NOT EXISTS finalPeptidePPMInfo_v1 ON finalPeptidePPMInfo( id,fractionID,scan,newTheoMass,newPPM,nTerm,finalPeptide,cTerm)")
    conn.commit()
    cursor.execute("DROP VIEW IF EXISTS v_finalPeptidePPMInfo")
    cursor.execute("DROP VIEW IF EXISTS v_cleanDataFDR")
    cursor.execute("DROP VIEW IF EXISTS v_modsPerScan")
    conn.commit()
    cursor.execute("CREATE VIEW v_finalPeptidePPMInfo AS select * from finalPeptidePPMInfo group by id,fractionID,scan,newTheoMass,newPPM,nTerm,finalPeptide,cTerm")
    cursor.execute("CREATE VIEW v_cleanDataFDR AS select * from cleanDataFDR group by id,fractionID,scan")
    cursor.execute("CREATE VIEW v_modsPerScan AS select * from modsPerScan group by id,fractionID,scan")
    conn.commit()

def getTaxonsInfo(conn):
    cursor=conn.cursor()    
    query="select * from taxonsInfo"
    taxionIDName={}
    cursor.execute(query)
    taxonsInfo=cursor.fetchall()
    taxonID=[]
    taxonName=[]
    for taxon in taxonsInfo:
        taxonID.append(taxon[1].strip())
        taxonName.append(taxon[2].strip())
        taxionIDName[str(taxon[1].strip())]=taxon[2].strip()
    return taxonID,taxonName,taxionIDName

def writeScans(conn,fout,outFile,fractionMapping,fractionID,outputPerFraction,FDRCutoff,logEMCutoff,DisplayProteinNum,writeFileCnt,metaProteome,nonMp,taxonIDs,taxonNames,taxionIDName):
    ...
    if (outputPerFraction=="yes"):
        ...
        writeHeader(foutPreFraction,nonMp)
    ...
    cursor=conn.cursor()
    padding=0
    cursor.execute("CREATE INDEX IF NOT EXISTS correctedILPeptide_IL5 ON correctedILPeptide(scan,fractionID)")    
    cursor.execute("CREATE INDEX IF NOT EXISTS result_IL5 ON result(top_result,scan,fraction_id)")    
    conn.commit()
    if nonMp=="1":
        conn.execute("DROP VIEW IF EXISTS v_nonMPResults")
        conn.execute("DROP VIEW IF EXISTS v_nonMPResultsAll")
        conn.commit()
        conn.execute("CREATE VIEW v_nonMPResults AS select r.id,r.scan,r.charge ,r.obs_mh,r.theo_mh,r.retention_time,r.ppm,r.alignment_score,r.spectrum_score,r.composite_score,r.em_probability,r.log_em_probability,il.contextILCorrected as context,r.mod_context,r.modified,r.mods,r.mod_ambig_edges,r.mod_ranges,il.proteins,r.matching_tag_length ,r.time_taken,r.de_novo_peptide,r.unmod_de_novo_peptide,r.de_novo_score,r.num_matches ,r.top_result,r.unique_sibling_peptides,r.context_mod_variants,r.num_mod_occurrences,r.spectral_counts ,r.abundance,r.fraction_id from result r, correctedILPeptide il where r.top_result=1 and il.scan=r.scan and il.fractionID=r.fraction_id")
        conn.commit()
        conn.execute("CREATE VIEW v_nonMPResultsAll AS select r.id,r.scan,r.charge,r.obs_mh,r.theo_mh,r.retention_time,r.ppm,r.alignment_score,r.spectrum_score,r.composite_score,r.em_probability,r.log_em_probability,r.context,r.mod_context,r.modified,r.mods,r.mod_ambig_edges,r.mod_ranges,r.Proteins,r.matching_tag_length,r.time_taken,r.de_novo_peptide,r.unmod_de_novo_peptide,r.de_novo_score,r.num_matches,r.top_result,r.unique_sibling_peptides,r.context_mod_variants,r.num_mod_occurrences,r.spectral_counts,r.abundance,r.fraction_id,c.FDR,c.newEM,f.newTheoMass,f.newPPm,f.nTerm,f.finalPeptide,f.cTerm,m.* from v_nonMPResults r left join v_finalPeptidePPMInfo f on r.id=f.id left join v_cleanDataFDR c on r.id=c.id left join v_modsPerScan m on r.id=m.id where r.top_result=1 group by r.scan,r.fraction_id,f.finalPeptide,r.proteins")
        conn.commit()
        query="select r.*, GROUP_CONCAT(Proteins,' ||| ') as allProteins from v_nonMPResultsAll r where r.fraction_id="+str(fractionID) +" group by r.scan,r.finalPeptide"       
    else:
        if metaProteome=="0":
            useTBL=" v_topResultNSAF"
        else:
            useTBL=" v_topResults"
        conn.execute("DROP VIEW IF EXISTS v_MPResults")
        conn.execute("DROP VIEW IF EXISTS v_MPResultsAll")
        conn.commit()
        conn.execute("CREATE VIEW v_MPResults AS select r.id,r.scan,r.charge ,r.obs_mh,r.theo_mh,r.retention_time,r.ppm,r.alignment_score,r.spectrum_score,r.composite_score,r.em_probability,r.log_em_probability,il.contextILCorrected as context,r.mod_context,r.modified,r.mods,r.mod_ambig_edges,r.mod_ranges,il.proteins,r.matching_tag_length ,r.time_taken,r.de_novo_peptide,r.unmod_de_novo_peptide,r.de_novo_score,r.num_matches ,r.top_result,r.unique_sibling_peptides,r.context_mod_variants,r.num_mod_occurrences,r.spectral_counts ,r.abundance,r.fraction_id,r.coveringSetSize,r.combinedNSAF from "+useTBL+" r, correctedILPeptide il where r.top_result=1 and il.scan=r.scan and il.fractionID=r.fraction_id ")
        conn.commit()
        conn.execute("CREATE VIEW v_MPResultsAll AS select r.id,r.scan,r.charge,r.obs_mh,r.theo_mh,r.retention_time,r.ppm,r.alignment_score,r.spectrum_score,r.composite_score,r.em_probability,r.log_em_probability,r.context,r.mod_context,r.modified,r.mods,r.mod_ambig_edges,r.mod_ranges,r.Proteins,r.matching_tag_length,r.time_taken,r.de_novo_peptide,r.unmod_de_novo_peptide,r.de_novo_score,r.num_matches,r.top_result,r.unique_sibling_peptides,r.context_mod_variants,r.num_mod_occurrences,r.spectral_counts,r.abundance,r.fraction_id,c.FDR,c.newEM,f.newTheoMass,f.newPPm,f.nTerm,f.finalPeptide,f.cTerm,r.coveringSetSize,r.combinedNSAF,m.* from v_MPResults r left join v_finalPeptidePPMInfo f on r.id=f.id left join v_cleanDataFDR c on r.id=c.id left join v_modsPerScan m on r.id=m.id where r.top_result=1 group by r.scan,r.fraction_id,f.finalPeptide,r.proteins")
        query="select r.*, GROUP_CONCAT(Proteins,' ||| ') as allProteins from v_MPResultsAll r where r.fraction_id="+str(fractionID) +" group by r.scan,r.finalPeptide"
    cursor.execute(query)
    scans=cursor.fetchall()
    print("start writing... %s"%(datetime.datetime.now()))
    curScan=""
    curFinalPeptide=""
    for scan in scans:
        parsimonyProtein=""
        parsimonytaxon=""
        taxonInfo=""
        if (curScan!=scan[1] and curFinalPeptide!=scan[37]):
            ...
            if (float(checkFDR)<=floatFDRCutoff or float(scan[11])>=floatlogEMCutoff):
                ...
                if nonMp=="1":
                    #protein=scan[162]
                    (protein)=processProteinForPrintout(scan[162],DisplayProteinNum)
                else:
                    #protein=scan[164]
                    (protein,parsimonyProtein,parsimonytaxon,taxonInfo)=processProteinForPrintout_MP(scan[164],DisplayProteinNum,taxonIDs,taxonNames,taxionIDName)
                theoMass=scan[34]
                ppm=scan[35]
                ...
                if nonMp=="0":
                    if scan[39]==None:
                        ...
                        curcoveringSetSize=scan[39]
                    if scan[40]==None:
                        curcombinedNSAF=''
                    else:
                        curcombinedNSAF=scan[40]
                if nonMp=="1":
                    writeStr=str(fractionMapping[curFractionID])+":"+str(scan[1])+"\t"+str(scan[2])+"\t"+str(scan[5])+"\t"+str(scan[3])+"\t"+str(theoMass)+"\t"+str(ppm)+"\t"+str(ppmRangeUpperBound)+"\t"+str(ppmRangeLowerBound)+"\t"+str(ppmInRange)+"\t"+str(curFDR)+"\t"+str(scan[10])+"\t"+str(scan[11])+"\t"+str(scan[8])+"\t"+str(scan[7])+"\t"+str(scan[9])+"\t"+str(scan[26])+"\t"+str(scan[27])+"\t"+str(scan[28])+"\t"+str(scan[12])+"\t"+str(scan[13])+"\t"+str(scan[15])+"\t"+str(scan[16])+"\t"+str(scan[17])+"\t"+str(protein)+"\t"+str(scan[21])+"\t"+str(scan[23])+"\t"+str(scan[19])+"\t"+str(scan[24])+"\t"+str(scan[36])+"\t"+str(scan[37])+"\t"+str(scan[38])
                    for i in range(42,161):
                        writeStr+="\t"+str(scan[i])
                    fout.write(writeStr+"\n")
                    if (outputPerFraction=="yes"):
                        foutPreFraction.write(writeStr+"\n")
                else:
                    writeStr=str(fractionMapping[curFractionID])+":"+str(scan[1])+"\t"+str(scan[2])+"\t"+str(scan[5])+"\t"+str(scan[3])+"\t"+str(theoMass)+"\t"+str(ppm)+"\t"+str(ppmRangeUpperBound)+"\t"+str(ppmRangeLowerBound)+"\t"+str(ppmInRange)+"\t"+str(curFDR)+"\t"+str(scan[10])+"\t"+str(scan[11])+"\t"+str(scan[8])+"\t"+str(scan[7])+"\t"+str(scan[9])+"\t"+str(scan[26])+"\t"+str(scan[27])+"\t"+str(scan[28])+"\t"+str(scan[12])+"\t"+str(scan[13])+"\t"+str(scan[15])+"\t"+str(scan[16])+"\t"+str(scan[17])+"\t"+str(protein)+"\t"+str(parsimonyProtein)+"\t"+str(parsimonytaxon)+"\t"+str(taxonInfo)+"\t"+str(scan[21])+"\t"+str(scan[23])+"\t"+str(scan[19])+"\t"+str(scan[24])+"\t"+str(scan[39])+"\t"+str(scan[40])+"\t"+str(scan[36])+"\t"+str(scan[37])+"\t"+str(scan[38])
                    for i in range(44,163):
                        writeStr+="\t"+str(scan[i])
                    fout.write(writeStr+"\n")
                    if (outputPerFraction=="yes"):
                        foutPreFraction.write(writeStr+"\n")
            curScan=scan[1]
            curFinalPeptide=scan[37]
    if (outputPerFraction=="yes"):
        foutPreFraction.close()
    return (writeFileCnt,fout)

def loadFractionInfo_old(fracMappingFile):
...
def loadFractionInfo(fracMappingFile):
    fileFractionMappingPeak={}
    fractionMapping={}
    fractionList=[]
    with open(mappingFile,'rb') as fractionMappingF:
        fileFractionMapping = pickle.load(fractionMappingF)
    for currFilledFractionNumber, currFilename, currFilledFractionNumberFromFile  in fileFractionMapping:
        fractionMapping[currFilledFractionNumber]=currFilename
        fractionList.append(int(currFilledFractionNumber))
        if currFilledFractionNumber[0]=="0":
            fileFractionMappingPeak[currFilledFractionNumberFromFile]=currFilledFractionNumber[1]
        else:
            fileFractionMappingPeak[currFilledFractionNumberFromFile]=currFilledFractionNumber
    return (fractionMapping,fractionList,fileFractionMappingPeak)
...
#python scripts/ParseResultsDB.py "/home/slin/TagGraph/sampleInputFiles/TG/mzML_output/" "/home/slin/TagGraph/sampleInputFiles/TG/TAG_GRAPH_Tryp_CysCarbam_MetOx.ini" "Yes" "0.01" "2" "5" "0"
    if (len(sys.argv)<6):
    ...
    for dirName,subdirList,fileList in os.walk(outputFolder): #walk and find topResult.tdv file
        for fname in fileList:
            if fname.endswith("_TopResults.tdv")>0:
    ...
    mpOutFile=outFile
    if metaProteome=="1":
        mpOutFile=outputFolder+"/unique_"+outfile
        
    #20190105 get the fraction info  
    ...
    mappingFile=outputFolder+"/fileFractionMapping.pck"
    (fractionMapping,fractionList,fileFractionMappingPeak)=loadFractionInfo(mappingFile)
    ...
    if metaProteome=="1":
        (taxonIDs,taxonNames,taxionIDName)=getTaxonsInfo(conn)
    else:
        taxonIDs=[]
        taxonNames=[]
        taxionIDName={}
    for fractionID in fractionList:
        ...
        insertBaseDataForPPMCalculation(conn,idList,scanList,chargeList,PPMList,newPPMList,EMProbabilityList,oneSubtractlg10EMList,finalPeptideList,tenPowerNegOneSubtractlg10EMList)
        if printMsg:
    ...
    for fractionID in fractionList:
        ...
        (writeFileCnt,fout)=writeScans(conn,fout,outFile,fractionMapping,fractionID,outputPerFraction,FDRCutoff,logEMCutoff,DisplayProteinNum,writeFileCnt,"0",nonMP,taxonIDs,taxonNames,taxionIDName)
    fout.close()
    os.remove(outFile)
    writeFileCnt=0
    if metaProteome=="1":
        fmpOut=open(mpOutFile,'w') #dummy file handle
        for fractionID in fractionList:
            print("Write File: Meta Proteome: fraction: %s @ %s"%(fractionID,datetime.datetime.now()))
            (writeFileCnt,fmpOut)=writeScans(conn,fmpOut,mpOutFile,fractionMapping,fractionID,outputPerFraction,FDRCutoff,logEMCutoff,DisplayProteinNum,writeFileCnt,"1",nonMP,taxonIDs,taxonNames,taxionIDName)
        fmpOut.close()
        os.remove(mpOutFile)
